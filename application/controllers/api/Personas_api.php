...
class Personas_api extends REST_Controller{
	...
	public function remove_delete($id = NULL)
	{
		...
		$dataAudi = array (
			...
			'tabla_afectada' => 'tb_persona',
			'campo_afectado'=> 'nombre_apellido, cedula, nacionalidad, sexo, fecha_nacimiento,email,',
			'informacion_campo' => $dataAudi1->nombre_apellido.",".$dataAudi1->cedula.",".$dataAudi1->nacionalidad.",".$dataAudi1->sexo.",".$dataAudi1->fecha_nacimiento.",".$dataAudi1->email
						);
		//Se guarda la informacion del nuevo curso en la tabla tb_auditoria 
		$this->persona_model->addAuditoria($dataAudi);	
	...
	public function save_post()
	{
		...
		//echo $id_persona->id_persona; die();
		$data2=array(
        ...
        $this->persona_model->AsignarRol($email,$username);
		...
		$dataAudi = array (
			...
			'tabla_afectada' => 'tb_persona',
			'campo_afectado'=> 'nombre_apellido, cedula, nacionalidad, sexo, fecha_nacimiento,email,',
			'informacion_campo' => $data['nombre_apellido'].",".$data['cedula'].",".$data['nacionalidad'].",".$data['sexo'].",".$data['fecha_nacimiento'].",".$data['email']
						);
		//Se guarda la informacion del nuevo curso en la tabla tb_auditoria 
		$this->persona_model->addAuditoria($dataAudi);
	...
	public function updateMMDB_post()
	{
		...
		$this->session->set_flashdata('message', '<div class="alert alert-success info" role="alert">
						<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
						<span class="sr-only">Error:</span> Usuario modificado de forma exitosa.
						<span class="glyphicon glyphicon-remove close" aria-hidden="true"></span></div>');
		redirect(base_url());
